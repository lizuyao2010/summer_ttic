...
if __name__=="__main__":
    ...
    if mode=="train":
        ...
        with open('../data/vocab.txt','r') as fr:
            dic=json.load(fr)
        cache={}
        with open("../data/sim.txt",'r') as fr, open('../data/train_sim.txt','w') as fw:
            i=1
            count=0
            for line in fr:
                print i
                # sent=[]
                question,questionEntity,p,answer=line.strip().split('\t')
                # pr=p.split('/')
                # type2=pr[1]
                # predicate=pr[-1]
            ...
            # json.dump(dic,vocab)
            # pickle.dump( dic, vocab )
            with open('../data/vocab_sim.txt','wb') as vocab, open('../data/state_sim.txt','w') as state:
                pickle.dump( dic, vocab )
                print >> state, len(dic), count 