...
local opt = lapp[[
   ...
   -b,--batchSize     (default 400)          batch size
   -m,--momentum      (default 0)           momentum, for SGD only
   -i,--maxIter       (default 3)           maximum nb of iterations per batch, for LBFGS
   --coefL1           (default 0)           L1 penalty on the weights
   --coefL2           (default 0)           L2 penalty on the weights
   -t,--threads       (default 8)           number of threads
   -d,--dimension     (default 65)         dimension of embedding
   -a,--randomSampling   (default false)       randomSampling
   -c,--candidates    (default 2)           number of candidates
   --margin           (default 3)           margin
   --threshold        (default 0.3)         threshold
]]
-- fix seed
torch.manualSeed(1)
...
if opt.dataset=="web" then
  trainData=torch.load('../data/train_web.bin')
  
  Vocab=225961
elseif opt.dataset=="fb" then
  trainData=torch.load('../data/train_random_2.bin')
  -- newsize=math.floor(trainData:size()[1]/1000*999)
  Vocab=2334431
elseif opt.dataset=="sim" then
  trainData=torch.load('../data/train_random_sim.bin')
  Vocab=3047601
else
...
if opt.network == '' then
  ...
  -- mlp1 = nn.Sequential()
  -- mlp1:add(nn.SparseLinear(Vocab,opt.dimension))
  -- mlp1:add(nn.Tanh())
...
if opt.dataset=="web" then
  testData = torch.load('../data/test_web.bin')
elseif opt.dataset=="fb" then
  testData = torch.load('../data/test_2.bin')
elseif opt.dataset=="sim" then
  testData = torch.load('../data/test_sim.bin')
else
