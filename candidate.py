...
if __name__=="__main__":
    ...
    if mode=="train":
        dic={}
        cache={}
        with open("data/webquestions.txt",'r') as fr, open('data/train.txt','w') as fw, open('data/vocab.txt','w') as vocab, open('data/state.txt','w') as state:
            i=1
            count=0
            for line in fr:
                print i
                question,answer,questionEntity=line.strip().split(' # ')                                
                candidates,answer_id=getCandidates('/en/'+questionEntity,answer.decode('utf-8').replace(u"_",u" "))
                if answer_id:
                    count+=1
