...
if opt.network == '' then
  ...
  kw=opt.kw
  mlp1:add(nn.TemporalConvolution(Vocab_word-1,opt.relation_dimension,kw,1))
  mlp1:add(nn.Max(1))
...
function shrink_question( x )
  ...
  local sent = torch.zeros(sent_len,Vocab_word-1)
  local j=1
  for i=2,n+1 do
    if x[i]==37 then
      j=j+1
    else
      sent[j][x[i]]=sent[j][x[i]]+1
    end
  end
  return sent
end
  ...
  function createSparseVector( x,n )
    ...
    for i=1,n do
      if tonumber(x[i])==Vocab_word then
        ...
        sent[j][tonumber(x[i])]=sent[j][tonumber(x[i])]+1
      end
    end
    return sent
  end
