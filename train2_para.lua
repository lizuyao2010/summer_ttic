...
local opt = lapp[[
   ...
   -o,--optimization  (default "ADAGRAD")       optimization: SGD | LBFGS 
...
function assignOnesRandomly(Vocab)
    local x={}
    for i=1,3 do
...
function worker()
   ...
   while true do
      ...
      local output = model:forward(t[1])
      local err = crit:forward(output, t[2])
      -- estimate df/dW
      local df_do = crit:backward(output, t[2])
      model:backward(t[1], df_do)
      -- update confusion
      local predict=(output[1]-output[2])[1]
      if predict>=margin then confusion:add(1, t[2]) else confusion:add(-1, t[2]) end

      --send some data back
      parallel.parent:send(err)
   end
end

-- define code for parent:
function parent(inputs,f)
   ...
   for i = 1,#inputs do
      ...
      t={inputs[i],targets[i]}
      parallel.children:send(t)
      err = parallel.children:receive()
      f=f+err
   end
