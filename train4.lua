...
function loadTrainSet( state_file, train_file )
  ...
  for i=1,trainDataSize do
      ...
      -- if opt.randomSampling then
      --   for m=1,opt.negativeSamples do
      --     local z=assignOnesRandomly(Vocab)
      --     local output=1
      --     local input={{x,y},{x,z}}
      --     trainData[j]= {input,output}
      --     j=j+1
...
Vocab,trainData=loadTrainSet("../data/state_2025750.txt","../data/train_all_new.txt")
...
function test(inPutFilename,outPutFileName)
  ...
  local state=io.input("../data/state_2025750.test.txt")
  local l=io.read("*line"):split(" ")
  state:close()
...
while true do
  -- train
  train(trainData)
  test("../data/test_2025750.txt",outPutFileName)
